...
const HARD_NUANCES = ["pagination", "idempotency", "multiPart", "deprecated", "businessRules", "graphqlFirst", "officialSdk"];
...
function publicSkillDirs(repoDir) {
...
async function checkManifests(ctx) {
  ...
  if (plugin && !plugin.__err) {
    ...
    const expectedSkillPath = `./skills/${pkg.name}`;
    const skillsIsCanonical = Array.isArray(plugin.skills) && plugin.skills.length === 1 && plugin.skills[0] === expectedSkillPath;
    if (skillsIsCanonical) {
      ctx.results.push(pass("manifest", "plugin.json exposes exactly one umbrella skill"));
    } else {
      ctx.results.push(fail("manifest", "plugin.json skills must expose exactly one umbrella skill", { expected: [expectedSkillPath], got: plugin.skills }));
    }

    const publicSkills = publicSkillDirs(ctx.dir);
    const extraPublicSkills = publicSkills.filter((name) => name !== pkg.name);
    if (extraPublicSkills.length === 0) {
      ctx.results.push(pass("manifest", "no extra public skill shards under skills/"));
    } else {
      ctx.results.push(fail("manifest", "extra public skill shards under skills/", { expected: pkg.name, extra: extraPublicSkills }));
    }
...
async function checkStructural(ctx) {
  ...
  // Generated repos expose exactly one umbrella skill at
  // skills/<pkg.name>/SKILL.md (+ references/).
  if (pkg.bin) {
    const singularPath = join(ctx.dir, `skills/${pkg.name}/SKILL.md`);
    const skillPath = existsSync(singularPath) ? singularPath : null;

    if (!skillPath) {
      ctx.results.push(fail("structural", `primary SKILL.md missing (looked at skills/${pkg.name}/)`));
    } else {
...
async function checkNuances(ctx) {
  ...
  if (nuances.graphqlFirst) {
    const cov = readJson(join(ctx.dir, "coverage.json"));
    const hasGraphqlCoverage = !cov.__err && Array.isArray(cov.endpoints) && cov.endpoints.some((e) => typeof e.path === "string" && /\/graphql(?:\.json)?#/.test(e.path));
    const hasGraphqlActions = /kind\s*:\s*["']graphql["']/.test(src);
    if (!hasGraphqlActions) ctx.results.push(fail("nuances", "graphqlFirst=true but no kind:\"graphql\" action defs found"));
    else if (!hasGraphqlCoverage) ctx.results.push(fail("nuances", "graphqlFirst=true but coverage.json has no /graphql(.json)# action paths"));
    else ctx.results.push(pass("nuances", "GraphQL-first action defs + coverage paths present"));

    if (!/\bgql\b/.test(src) || !/introspect/.test(src)) {
      ctx.warnings.push("graphqlFirst=true but raw gql and/or introspection helper was not detected");
    }
  }

  if (nuances.officialSdk) {
    const deps = { ...(pkg.dependencies || {}), ...(pkg.devDependencies || {}) };
    const hasDeps = Object.keys(deps).length > 0;
    const hasWhy = existsSync(join(ctx.dir, "knowledge/why-official-sdk.md"));
    if (!hasDeps) ctx.results.push(fail("nuances", "officialSdk=true but package.json declares no dependencies"));
    else if (!hasWhy) ctx.results.push(fail("nuances", "officialSdk=true but knowledge/why-official-sdk.md missing"));
    else ctx.results.push(pass("nuances", "SDK/API-client dependency + knowledge rationale present"));
  }
