...
async function checkStructural(ctx) {
  ...
  // Preferred: one umbrella skill at skills/<pkg.name>/SKILL.md (+ references/).
  // Legacy: skills/<pkg.name>-workflow/SKILL.md (old modular primary).
  // Legacy: skills/<api-slug>/SKILL.md (pre-modular single folder).
  if (pkg.bin) {
    ...
    let skillPath = null;
    if (existsSync(singularPath)) skillPath = singularPath;
    else if (existsSync(modularPath)) skillPath = modularPath;
    else if (existsSync(legacyPath)) skillPath = legacyPath;

    if (!skillPath) {
      ctx.results.push(fail("structural", `primary SKILL.md missing (looked at skills/${pkg.name}/, skills/${pkg.name}-workflow/, skills/${apiSlug}/)`));
    } else {
