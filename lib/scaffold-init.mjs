...
import { readdirSync, readFileSync, writeFileSync, mkdirSync, statSync, existsSync, copyFileSync, chmodSync } from "node:fs";
...
export function scaffoldInit({ apiName, target = process.cwd(), exemplarDir = EXEMPLAR_DIR }) {
  ...
  // Belt-and-suspenders: ensure the bin file is executable even on filesystems
  // that drop the mode (e.g. tarball unpack, some sync tools). The exemplar
  // ships at 0755 in git, but we re-apply here so the generated repo never
  // requires a manual `chmod +x` after `npm install && npm link`.
  const binPath = join(destRoot, "bin", `${apiName}-cli.mjs`);
  if (existsSync(binPath)) {
    try { chmodSync(binPath, 0o755); } catch { /* non-POSIX */ }
  }
